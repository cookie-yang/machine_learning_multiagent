...
class GradingAgent(Agent):
    ...
    def getAction(self, state):
        ...
        #print(studentAction, optimalActions)
...
class TicTacToeTest(testClasses.TestCase):
    def __init__(self, question, testDict):
        super(TicTacToeTest, self).__init__(question, testDict)
        self.player1 = testDict['player1']
        self.player2 = testDict['player2']
        self.iteration = int(testDict['iteration'])
        self.maxTimeOut = int(testDict['max_timeout'])
    ...
    def getSolInfo(self, solveTicTacToe):
        ...
        for i in range(self.iteration):
            ...
            while True:
                if agentIndex == 0: 
                    timed_func = TimeoutFunction(AIPlayer.getAction, int(self.maxTimeOut))
                    try:
                        start_time = time.time()
                        action = timed_func(gameState, self.gameRules)
                    except TimeoutFunctionException:
                        print("***\tERROR: Player %d timed out on a single move, Max %d Seconds!***\t" % (agentIndex+1, self.maxTimeOut))
                        return 0, False
                else:
            ...
            if agentIndex == 1:
                solutionCounter += 1
            print('****Player 1 wins %d/%d games' % (i, self.iteration), end="\r")

        return solutionCounter, None

    def execute(self, grades, moduleDict, solutionDict):
        solveTicTacToe = moduleDict['solveTicTacToe']
        solutionCounter, error = self.getSolInfo(solveTicTacToe)
        if error != None:
            grades.addMessage('FAIL: %s' % self.path)
            grades.addMessage('%s' % error)
            return False
        if solutionCounter == int(self.iteration):
            grades.addMessage('PASS: %s' %self.path)
            grades.addMessage('\t Solved:\t\t%s/%s' % (solutionCounter, self.iteration))
            grades.addMessage('\t Player1 Wins all the games:\t\t')
            return True
        else:
            grades.addMessage('FIAL: %s' % self.path)
            grades.addMessage('\t Solved:\t\t%s/%s' % (solutionCounter, self.iteration))
            grades.addMessage('\t Player1 loss some of the games:\t\t')
            return False
